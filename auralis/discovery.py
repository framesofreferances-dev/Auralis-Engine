...
def score_candidate(query: str, candidate: Candidate) -> float:
    ...
    provider_bonus = {"jamendo": 10.0, "youtube": 0.0}.get((candidate.provider or "").lower(), 0.0)
    return round(rel * 50.0 + fresh * 25.0 + pop * 25.0 + provider_bonus, 2)
...
def _jamendo_queries(query: str) -> list[str]:
    ...
    preferred = [
        "cinematic", "ambient", "instrumental", "eerie", "ethereal",
        "atmospheric", "drone", "synth", "electronic", "dark", "glitch",
        "metallic", "brass", "foggy", "calm",
    ]
...
def _search_jamendo_resilient(query: str, limit: int) -> list[Candidate]:
    ...
    for attempt, jamendo_query in enumerate(_jamendo_queries(query), start=1):
        try:
            ...
            if candidates:
                if attempt > 1:
                    logger.info("Jamendo fallback query succeeded: %s", jamendo_query)
                return candidates
            logger.info("Jamendo returned no candidates for query: %s", jamendo_query)
        except (JamendoConfigurationError, JamendoError) as exc:
            last_error = exc
            if isinstance(exc, JamendoConfigurationError):
                raise
            logger.warning("Jamendo query failed (%s): %s", jamendo_query, exc)
